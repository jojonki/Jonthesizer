...
def A_weighting(frequencies, *, min_db=-80.0):  # pylint: disable=invalid-name
    ...
    weights = 2.0 + 20.0 * (
        np.log10(const[0]) + 2 * np.log10(f_sq) - np.log10(f_sq + const[0]) -
        np.log10(f_sq + const[1]) - 0.5 * np.log10(f_sq + const[2]) -
...
def B_weighting(frequencies, *, min_db=-80.0):  # pylint: disable=invalid-name
    ...
    weights = 0.17 + 20.0 * (
        np.log10(const[0]) + 1.5 * np.log10(f_sq) - np.log10(f_sq + const[0]) -
        np.log10(f_sq + const[1]) - 0.5 * np.log10(f_sq + const[2]))
...
def C_weighting(frequencies, *, min_db=-80.0):  # pylint: disable=invalid-name
    ...
    weights = 0.062 + 20.0 * (np.log10(const[0]) + np.log10(f_sq) -
...
def D_weighting(frequencies, *, min_db=-80.0):  # pylint: disable=invalid-name
    ...
    const = np.array([8.3046305e-3, 1018.7, 1039.6, 3136.5, 3424, 282.7,
                      1160])**2.0
    weights = 20.0 * (0.5 * np.log10(f_sq) - np.log10(const[0]) + 0.5 *
                      (+np.log10(
                          (const[1] - f_sq)**2 + const[2] * f_sq) - np.log10(
                              (const[3] - f_sq)**2 + const[4] * f_sq) -
                       np.log10(const[5] + f_sq) - np.log10(const[6] + f_sq)))
...
def multi_frequency_weighting(frequencies, *, kinds="ZAC", **kwargs):
    ...
    **kwargs : keywords to pass to the weighting function.
    ...
    """
    return np.stack(
        [frequency_weighting(frequencies, kind=k, **kwargs) for k in kinds],
        axis=0)
